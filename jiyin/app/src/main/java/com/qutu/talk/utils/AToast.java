...
public class AToast extends Toast {
    ...
    private static void initToast(Context context, CharSequence text) {
        try {
            ...
            // 由layout文件创建一个View对象
            View layout = inflater.inflate(R.layout.toast2_layout, null);
    ...
    private static void showToast(Context context, CharSequence text, int time, int imgType) {
        ...
        if (imgType == TYPE_HIDE) {
            ...
            if (imgType == TYPE_TRUE) {
                toast_img.setBackgroundResource(R.drawable.my_package_shape); //对号
            } else {
                toast_img.setBackgroundResource(R.drawable.hui_label_item_shape); //叉号
            }
