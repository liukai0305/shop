...
public class MiniCircleRecyclerView  extends RecyclerView implements View.OnClickListener  {
    ...
    private final MiniCircleRecyclerView.CenterRunnable mCenterRunnable = new MiniCircleRecyclerView.CenterRunnable();
    private ItemViewMode mViewMode;
    private boolean mNeedCenterForce;
    private boolean mNeedLoop = true;
    private CircleRecyclerView.OnCenterItemClickListener mCenterItemClickListener;
    private View mCurrentCenterChildView;
    private CircleRecyclerView.OnScrollListener mOnScrollListener;
    private boolean mFirstOnLayout;
    private boolean mFirstSetAdapter = true;
    private CircleRecyclerView.CurrentItemCallback callback;
    ...
    public MiniCircleRecyclerView(Context context) {
        this(context, null);
    }

    public MiniCircleRecyclerView(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public MiniCircleRecyclerView(Context context, @Nullable AttributeSet attrs, int defStyle) {
...
//            scrollToPosition(DEFAULT_SELECTION);
        ...
        if (mViewMode != null) {
            ...
            for (int i = 0; i < count; ++i) {
                ...
                if (v == mCurrentCenterChildView)
                    v.setTag(me.khrystal.library.R.string.tag_is_center, true);
                else
                    v.setTag(me.khrystal.library.R.string.tag_is_center, false);
     ...
     * @param listener
     */
    public void setOnCenterItemClickListener(CircleRecyclerView.OnCenterItemClickListener listener) {
    ...
    public void setOnScrollListener(CircleRecyclerView.OnScrollListener listener) {
    ...
    public void setCurrentItemCallback(CircleRecyclerView.CurrentItemCallback callback) {
        this.callback = callback;
    }

    public interface CurrentItemCallback{
        void onItemInCenter(View v);
    }
}
