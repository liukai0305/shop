...
import static com.qutu.talk.utils.Constant.QUXIAOHUANYINGKUANG;
...
import static com.qutu.talk.utils.Constant.ZHUANGBANHUANYINGBIAOQIAN;

/**
 * 我的背包----进场特效
 * 老王
 */
public class BeiBaoIntoSEFragment extends MyBaseArmFragment {
    @Inject
    ...
    protected void visibleToLoadData() {
        ...
        mAdapter.setOnItemClickListener((adapter, view, position) -> {
            ...
            if (mOnPageChangeLister != null) {
                mOnPageChangeLister.OnListener(mAdapter.getData().get(position), 2, 3);
            }
        });
    }
    ...
    private void getMyPackBaoShi() {
        RxUtils.loading(commonModel.my_pack("6"), this)
    ...
    public void receiveMsg(FirstEvent event) {
        ...
        if (ZHUANGBANHUANYINGBIAOQIAN.equals(tag)) {
            mPullRefreshBean.setRefresh(mPullRefreshBean, smartRefreshLayout);
            getMyPackBaoShi();
        } else if (QUXIAOHUANYINGKUANG.equals(tag)) {
            mPullRefreshBean.setRefresh(mPullRefreshBean, smartRefreshLayout);
            getMyPackBaoShi();
        }else if (QIEHUANTWO.equals(tag)){
            if ("3".equals(event.getMsg())){
                for (int a = 0; a < mDataList.size(); a++) {
                    mDataList.get(a).setSelect(false);
                }
                mAdapter.notifyDataSetChanged();
