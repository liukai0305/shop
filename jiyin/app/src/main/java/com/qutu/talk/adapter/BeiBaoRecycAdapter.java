...
public class BeiBaoRecycAdapter extends BaseQuickAdapter<MyPackBean.DataBean, BaseViewHolder> {
    public BeiBaoRecycAdapter(int layoutResId, @Nullable List<MyPackBean.DataBean> data) {
        super(layoutResId, data);
    }
//    public BeiBaoRecycAdapter() {
//        super();
//    }
    ...
    protected void convert(@NonNull BaseViewHolder helper, MyPackBean.DataBean item) {
        ...
        if ((helper.getPosition() + 1) % 4 == 1) {
            helper.getView(R.id.beibao_recyc_back).setBackgroundResource(R.mipmap.bag_bg_changzs);
            helper.getView(R.id.name).setBackgroundResource(R.mipmap.bag_label_zi);
        } else if ((helper.getPosition() + 1) % 4 == 2) {
            helper.getView(R.id.beibao_recyc_back).setBackgroundResource(R.mipmap.bag_bg_changfs);
            helper.getView(R.id.name).setBackgroundResource(R.mipmap.bag_label_fen);
        } else if ((helper.getPosition() + 1) % 4 == 3) {
            helper.getView(R.id.beibao_recyc_back).setBackgroundResource(R.mipmap.bag_bg_changls);
            helper.getView(R.id.name).setBackgroundResource(R.mipmap.bag_label_lan);
        } else if ((helper.getPosition() + 1) % 4 == 0) {
            helper.getView(R.id.beibao_recyc_back).setBackgroundResource(R.mipmap.bag_bg_changhs);
        ...
        if (item.isSelect()) {
            helper.getView(R.id.beijing).setBackgroundResource(R.mipmap.bag_xz_bk);
        } else {
