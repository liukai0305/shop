...
import com.qutu.talk.adapter.dashen.RecomHomePageAdapter;
 ...
 * 描述:首页语音娱乐
 */
public class IndexPeiwanFragment extends HeaderViewPagerFragment {
...
//    private RecomAdapter adapter;
    ...
    private int id;
    private RecomHomePageAdapter topAdapter;
    private PeiwanHoriAdapter horizontalAdapter;
...
//    private boolean isPrepared;
    ...
    List<MainHomePageBean.DataBean> mDataList1 = new ArrayList<>();
...
//        roomTypeResult = (RoomTypeResult) getArguments().getSerializable("categorRoomBean");
    ...
    protected void visibleToloadData() {
        //求陪玩
        topAdapter = new RecomHomePageAdapter(R.layout.recom_home_page_item, mDataList, getActivity());
        myList1.setLayoutManager(new GridLayoutManager(getActivity(),2));
        myList1.setAdapter(topAdapter);
        //陪玩
        horizontalAdapter = new PeiwanHoriAdapter( mDataList1,mContext);
        ...
        topAdapter.setOnItemClickListener((adapter, view, position) -> {
            enterData(String.valueOf(topAdapter.getData().get(position).getUser_id()), "", commonModel, 1, topAdapter.getData().get(position).getHeadimgurl());
        });

        horizontalAdapter.setOnItemClickListener((adapter, view, position) -> {
            enterData(String.valueOf(horizontalAdapter.getData().get(position).getUser_id()), "", commonModel, 1, horizontalAdapter.getData().get(position).getHeadimgurl());
        });
    }
...
//        loadData(refreshLayout);
    ...
    public static IndexPeiwanFragment getInstance() {
//    public static RecomFragment getInstance(int tag, RoomTypeResult roomTypeResult) {
        IndexPeiwanFragment fragment = new IndexPeiwanFragment();
...
//                    }
//                });
        RxUtils.loading(commonModel.getSkillGodList("1", "", "", "", "", mPullRefreshBean.pageIndex), this)
                .subscribe(new ErrorHandleSubscriber<MainHomePageBean>(mErrorHandler) {
                    ...
                    public void onNext(MainHomePageBean mainHomePageBean) {
                        ...
                        List<MainHomePageBean.DataBean> topData=new ArrayList<>();
                        for (int i = 0; i < (data.size()>=4?4:data.size()); i++) {
                            topData.add(data.get(i));
                        }
                        //设置数据，包括加载更多，下拉刷新，无数据的情况都处理过了
...
//                        refreshLayout.setEnableLoadMore(false);
                        new DealRefreshHelper<MainHomePageBean.DataBean>().dealDataToUI(refreshLayout, topAdapter, noData, topData, mDataList, mPullRefreshBean);
                    }

                    @Override
                    public void onError(Throwable t) {
                        super.onError(t);
                        new DealRefreshHelper<MainHomePageBean.DataBean>().dealDataToUI(refreshLayout, topAdapter, noData, new ArrayList<>(), mDataList, mPullRefreshBean);
                    }
                });
    ...
    private void hotDataNew2(SmartRefreshLayout refreshLayout) {
        RxUtils.loading(commonModel.getSkillGodList("", "", "", "", "", mPullRefreshBean.pageIndex), this)
                .subscribe(new ErrorHandleSubscriber<MainHomePageBean>(mErrorHandler) {
                    @Override
                    public void onNext(MainHomePageBean hotBean) {
                        List<MainHomePageBean.DataBean> data = hotBean.getData();
