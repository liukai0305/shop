...
public class CommDynamicAdapter extends BaseQuickAdapter<RecommendedDynamicBean.DataBean, BaseViewHolder> {
    //该变量如果设置成全局的会发生数据紊乱
...
//        this.mCountDownCounters = new SparseArray<>();
    ...
    @Override
    protected void convert(BaseViewHolder helper, RecommendedDynamicBean.DataBean item) {
        ...
        if (item.getIs_follow() == 0) {
            helper.setText(R.id.guanzhu_btn, "关注");
//            helper.setVisible(R.id.tv_to_attention, true);
        } else {
            helper.setText(R.id.guanzhu_btn, "已关注");
//            helper.setVisible(R.id.tv_to_attention, false);
        ...
        if (item.getVip_level() == 0) {
            helper.getView(R.id.dy_rank_image).setVisibility(View.GONE);
        } else {
            helper.getView(R.id.dy_rank_image).setVisibility(View.VISIBLE);
            JudgeImageUtil.noZeroVIP(item.getVip_level(), helper.getView(R.id.dy_rank_image));
        }
            ...
            helper.getView(R.id.dy_voice).setOnClickListener(v -> {
                ...
                if (!item.isPlay()) {
                    ...
                    if (mSingleCountDownTimer != null && !mHasFinishTimer) {
                        ...
                        MediaManager.pause();
                        List<RecommendedDynamicBean.DataBean> list = getData();
                        int size = list.size();
                        int position = 0;
                        for (int i = 0; i < size; i++) {
                            RecommendedDynamicBean.DataBean dynamicsBean = list.get(i);
...
//                            mCountDownCounters.put(helper.getView(R.id.dy_voice_time).hashCode(), countDownTimer);
        ...
        if (!tags_str.isEmpty()) {
            helper.getView(R.id.dy_label).setVisibility(View.VISIBLE);
            tagsList.clear();
...
//        });
    ...
    @Override
    protected void convertPayloads(@NonNull BaseViewHolder helper, RecommendedDynamicBean.DataBean item, @NonNull List<Object> payloads) {
