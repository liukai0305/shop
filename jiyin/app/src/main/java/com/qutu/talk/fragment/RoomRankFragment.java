...
//    TextView roomRankYue;
    ...
    private int id, type; //  ，1房间金锐榜3房间星锐榜 type
...
//        roomRankYue.setSelected(false);
    ...
    private void setTop(List<RoomRankBean.DataBean.TopBean> topBeanList) {
        ...
        if ("".equals(topBeanList.get(0).getName())) {
            ...
            roomRankZuan1.setText(topBeanList.get(0).getMizuan());
            img1.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    Intent intent1 = new Intent(getActivity(), MyPersonalCenterTwoActivity.class);
                    if (topBeanList.get(0).getId() == UserManager.getUser().getUserId()+"") {
                        intent1.putExtra("sign", 0);
                        intent1.putExtra("id", "");
                        intent1.putExtra("isRoom", true);
                    } else {
                        intent1.putExtra("sign", 1);
                        intent1.putExtra("id", topBeanList.get(0).getId() );
                        intent1.putExtra("isRoom", true);
                    }
                    ArmsUtils.startActivity(intent1);
                    boolean mCanReture = true;
                }
            });
            if (type == 1) {
                roomRankZuanTit1.setText("财富值:");
                roomRankZuan1.setTextColor(getActivity().getResources().getColor(R.color.color_FFBA1C));
            } else {
                roomRankZuanTit1.setText("魅力值:");
    ...
    @OnClick({R.id.room_rank_ri, R.id.room_rank_zhou})
