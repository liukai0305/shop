 ...
 * 老王
 */
public class BeiBaoHeadKuangFragment extends MyBaseArmFragment {
    @Inject
    ...
    protected void visibleToLoadData() {
        ...
        mAdapter.setOnItemClickListener((adapter, view, position) -> {
            ...
            if (mOnPageChangeLister != null) {
                mOnPageChangeLister.OnListener(mAdapter.getData().get(position), 1, 1);
            }
    ...
    private void getMyPackBaoShi() {
        RxUtils.loading(commonModel.my_pack("4"), this)
    ...
    public void receiveMsg(FirstEvent event) {
        ...
        if (ZHUANGBANCHENGGONGTOUXIANG.equals(tag)) {
            mPullRefreshBean.setRefresh(mPullRefreshBean, smartRefreshLayout);
            getMyPackBaoShi();

        } else if (QUXIAOZHUANGBANTOUXIANG.equals(tag)) {
            mPullRefreshBean.setRefresh(mPullRefreshBean, smartRefreshLayout);
            getMyPackBaoShi();
