 ...
 */
@ActivityScope
public class AdminUserAdapter2 extends MyBaseAdapter<AdminUser.DataBean.VisitorBean> {

    private Context context;

    public AdminUserAdapter2(Context context) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        VH.textCount.setOnClickListener(v -> {//删除
            if(context instanceof SetAdminActivity){
                ((SetAdminActivity) context).is_admin(String.valueOf(list_adapter.get(position).getId()),0);
            }
        });
        return convertView;
    }
