 ...
 */
@ActivityScope
public class AdminFamilyUserAdapter2 extends MyBaseAdapter<FamilyUser> {

    private Context context;

    public AdminFamilyUserAdapter2(Context context) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        VH.textCount.setOnClickListener(v -> {//删除
            if(context instanceof SetFamilyAdminActivity){
                ((SetFamilyAdminActivity) context).is_admin(String.valueOf(list_adapter.get(position).getFamily_user_id()),0);
            }
        });
        return convertView;
    }
