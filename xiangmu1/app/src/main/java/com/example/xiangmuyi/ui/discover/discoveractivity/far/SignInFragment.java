...
public class SignInFragment extends BaseFragment<IDiscover.EsperienPersenter> implements IDiscover.EsperienView {
        ...
        public void getQian(QianDaoBean result) {
            Log.d(TAG, "getLevelReturn: " + result.getData().getSignTop().getList().size());
            if (result.getStatus().getStatusCode() == 100) {
                List<ListBean> list = result.getData().getSignTop().getList();
                listBeans.addAll(list);
                richAdapter = new RichAdapter(listBeans,context);
                richAdapter.setType(2);
