...
public class LevelFragment extends BaseFragment<IDiscover.EsperienPersenter> implements IDiscover.EsperienView {
    ...
    public void getEsperien(EsperienBean result) {
        Log.d(TAG, "getEsperien: "+result.toString());
        if (result.getStatus().getStatusCode() == 100) {
            List<ListBean> list = result.getData().getExpTop().getList();
            listBeans.addAll(list);
            richAdapter = new RichAdapter(listBeans,context);
            richAdapter.setType(1);
    ...
    @Override
    public void getQian(QianDaoBean discover) {
